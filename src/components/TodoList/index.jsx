...
function ToDoList() {
  ...
  const { user } = useAuth();
  const { tasks, deleteTask, updateTask, taskLoading } =
  ...
  const checkNoTask = user
    ? !taskLoading && allTodos.length === 0
    : !isLoading && searchedTodos.length === 0 && searchValue.length < 1;
  const checkNoFound = user
    ? !taskLoading && allTodos.length > 0 && searchedTodos.length === 0
          ...
          `}
            >
              <ToDoItem
                ...
                onCompleted={() =>
                  user
                    ? updateTask(ToDo.id, ToDo.completed,user)
                    : completeToDoHandler(ToDo.id)
                }
                handleRemove={() =>
                  user ? deleteTask(ToDo.id,user) : handleRemove(ToDo.id)
                }
              />
            </li>
          );
